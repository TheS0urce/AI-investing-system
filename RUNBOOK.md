...
Fetch the read-only Alpaca paper market clock:
```bash
curl -s "http://127.0.0.1:8001/broker/paper/clock" \
  -H "X-API-Key: $(grep '^AI_API_KEY=' .env | cut -d= -f2-)"
.venv/bin/python scripts/paper_market_session_plan.py
curl -s "http://127.0.0.1:8001/broker/paper/session_plan" \
  -H "X-API-Key: $(grep '^AI_API_KEY=' .env | cut -d= -f2-)"
```
...
curl -s "http://127.0.0.1:8001/broker/paper/market_open_preflight" \
  -H "X-API-Key: $(grep '^AI_API_KEY=' .env | cut -d= -f2-)"
curl -s "http://127.0.0.1:8001/broker/paper/next_action" \
  -H "X-API-Key: $(grep '^AI_API_KEY=' .env | cut -d= -f2-)"
```
...
The dashboard exposes this as `Run Market-Open Preflight` and `Show Next Paper Action`.
...
For unattended market-open validation, prefer the local macOS LaunchAgent rather than
Codex automations, because the automation sandbox may not be allowed to reach
`127.0.0.1`.

```bash
./scripts/install_market_open_watch_agent.sh
launchctl list | grep com.aiinvesting.market-open-watch
tail -f logs/market_open_watch_agent.log
```
