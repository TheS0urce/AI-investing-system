...
def test_session_plan_waits_when_market_closed():
    ...
    assert plan["seconds_until_next_open"] == 219_278
    assert plan["time_until_next_open"] == "2d 12h 54m"
...
def test_session_plan_recommends_watch_when_market_open():
    ...
    assert plan["seconds_until_next_close"] == 21_600
    assert plan["time_until_next_close"] == "6h 0m"
...
def test_format_duration_handles_short_and_long_values():
    assert paper_market_session_plan.format_duration(59) == "0m"
    assert paper_market_session_plan.format_duration(3_600) == "1h 0m"
    assert paper_market_session_plan.format_duration(90_000) == "1d 1h 0m"
